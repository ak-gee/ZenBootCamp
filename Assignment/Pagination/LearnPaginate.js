...
function EnableDisable() {
  ...
  if (currentPage === numberOfPages)
  {
    ...
    // document.getElementById("Next").disabled = ((currentPage !== numberOfPages )? false : true);
}

function Start() {
 
    ctn=make_new('div','container-fluid','','')
    document.body.append(ctn)

    var table= make_new('table','table','table')
    ctn.append(table)

    FillTable(list,numberPerPage)
    currentPage=1;
    AddButtons();
...
function AddButtons()
{
    ...
    var btnFirst= make_new('button','btn btn-primary','First','First');
    var btnNext= make_new('button','btn btn-primary','Next','Next');
    var btnPrev = make_new('button','btn btn-primary','Previous','Previous');
    var btnLast = make_new('button','btn btn-primary','Last','Last');
  ...
  function FillTable(tabledata,size)
  {
    ...
    function paginate(jsonList,currentPage,size)
    {
      let start = (currentPage-1)*size
      let end = start+size;
   
       /* Break Table for Display */
       dispTable = jsonList.slice(start,end)
