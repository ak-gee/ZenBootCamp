...
/*Build Body with the Table*/
  idData=[];
  for (key in EmployeeList[0])
  idData.push(key)

  var ctn=make_new('div','container-fluid')
  ...
  /*Global Variables for Design*/
    var size=10;
    var noPages = Math.ceil(EmployeeList.length/size);
    var noButtons = 5;
    var currPage=1;

    var Left = 0;
    var Right = 0;
    var btnholder = make_new('div')
    ctn.append(btnholder)
  

  FillTable(EmployeeList,size);
  DisplayButtons();
  MakeButtons(noPages);
  ...
  for(let i=0;i<size;i++)
  {
      tr = table.insertRow(-1)
        for(let j=0;j<idData.length;j++)
        {
            ...
            td.innerHTML=tabledata[i][idData[j]]
...
/* Create TableSize per page or Paginate */
function paginate(EmployeeList,currPage,size){
    let start = (currPage-1)*size
     ...
     /* Break Table for Display */
     dispTable = EmployeeList.slice(start,end)
    FillTable(dispTable,size)
     return dispTable
}

/*Decide what buttons to display based on current page*/
function DisplayButtons(){
    
    Left = currPage-Math.floor(noButtons/2);
    Right = currPage+Math.floor(noButtons/2);
    if (Left  < 1){
        Left =  1;
        Right = noButtons;
    }
...
function MakeButtons(noPages){
   
    btnholder.innerHTML=""
    console.log(Left,Right)
    /**First Button */
    var btnFirst= make_new('button','btn btn-primary button1','F1','<First');        
        
    btnFirst.style.width='200px';
    btnholder.append(btnFirst);
    addBtnEventFL(btnFirst,1)
    /*Numbered Buttons */
    // for(let pg=1;pg<=noPages;pg++)
    for(let pg=Left;pg<=Right;pg++)
    {
        var btn=[];
        btn[pg]= make_new('button','btn btn-primary','btn'+pg,pg);
        btnholder.append(btn[pg]);
         addBtnEvent(btn[pg],pg)
         btn[pg].innerHTML =pg;
    ...
    /* LAst Button */
    if (currPage != noPages){
        var btnLast= make_new('button','btn btn-primary','F'+noPages,noPages);
 
        btnLast.innerHTML='Last>'
        btnLast.style.width='200px';
        btnholder.append(btnLast);
        addBtnEventFL(btnLast,noPages)
    }
    console.log(Left,Right)

}
   /*Create Button Events */
function addBtnEventFL(btn,pg){
    ...
    paginate(EmployeeList,currPage,size)
     
    });
}
 /*Create First and Last Button Events */
function addBtnEvent(btn,pg){
